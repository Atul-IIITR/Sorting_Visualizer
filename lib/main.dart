...
class _MainWidgetState extends State<MainWidget> {
      ...
      child: Scaffold(
        appBar: AppBar(actions: [
          Center(
            child: Text(
              "$multiplier x Speed",
              style: const TextStyle(color: Colors.black12, fontSize: 20, fontWeight: FontWeight.w700),
            ),
          ),
          IconButton(
            onPressed: () {
  ...
  Future<void> bubbleSort() async {
    for (int i = 0; i < array.length; i++) {
      array[i].color = Colors.yellowAccent;
      for (int j = 0; j < array.length - i - 1; j++) {
        if (array[j].value > array[j + 1].value) {
          array[j].color = Colors.redAccent;
          array[j + 1].color = Colors.redAccent;
          ...
          array[j].color = Colors.yellowAccent;
          array[j + 1].color = Colors.yellowAccent;
        }
      }
    }
  }
  ...
  Future<void> selectionSort() async {
    for (int i = 0; i < array.length; i++) {
      ...
      array[minIndex].color = Colors.redAccent;
      array[i].color = Colors.redAccent;
      await Future.delayed(
        Duration(milliseconds: speed),
        () {
          setState(() {
      ...
      array[minIndex].color = Colors.yellowAccent;
      array[i].color = Colors.yellowAccent;
    }
  }
  ...
  Future<void> merge(int left, int mid, int right) async {
    ...
    while (i < n1 && j < n2) {
      if (leftArray[i].value <= rightArray[j].value) {
        leftArray[i].color = Colors.redAccent;
        rightArray[j].color = Colors.redAccent;
        await Future.delayed(
          Duration(milliseconds: speed),
          () {
            setState(() {
        ...
        leftArray[i].color = Colors.yellowAccent;
        rightArray[j].color = Colors.yellowAccent;
        i++;
      } else {
        leftArray[i].color = Colors.redAccent;
        rightArray[j].color = Colors.redAccent;
  ...
  Future<void> heapSort() async {
    ...
    for (int i = array.length - 1; i >= 0; i--) {
      array[0].color = Colors.redAccent;
      array[i].color = Colors.redAccent;
      await Future.delayed(
        Duration(milliseconds: speed),
        () {
      ...
      array[0].color = Colors.yellowAccent;
      array[i].color = Colors.yellowAccent;
      await heapify(i, 0);
    }
  }
  ...
  Future<void> heapify(int n, int i) async {
    ...
    if (largest != i) {
      array[i].color = Colors.redAccent;
      array[largest].color = Colors.redAccent;
      await Future.delayed(
        Duration(milliseconds: speed),
        () {
          setState(() {
      ...
      array[i].color = Colors.yellowAccent;
      array[largest].color = Colors.yellowAccent;
  ...
  Future<int> partition(int low, int high) async {
    ...
    array[i + 1].color = Colors.redAccent;
    array[high].color = Colors.redAccent;
    await Future.delayed(
      Duration(milliseconds: speed),
      () {
        setState(() {
    ...
    array[i + 1].color = Colors.yellowAccent;
    array[high].color = Colors.yellowAccent;
    return i + 1;
  }
  ...
  Future<void> bucketSort() async {
    ...
    for (int i = 0; i < buckets.length; i++) {
      for (int j = 0; j < buckets[i].length; j++) {
        buckets[i][j].color = Colors.yellowAccent;
        int k = j - 1;
        while (k >= 0 && buckets[i][k].value > buckets[i][k + 1].value) {
          buckets[i][k].color = Colors.redAccent;
          buckets[i][k + 1].color = Colors.redAccent;
          await Future.delayed(
            Duration(milliseconds: speed),
            () {
              setState(() {
          ...
          buckets[i][k].color = Colors.yellowAccent;
          buckets[i][k + 1].color = Colors.yellowAccent;
          k--;
        }
      }
    }
  ...
  Future<void> countSort() async {
    ...
    for (int i = 0; i < count.length; i++) {
      while (count[i] > 0) {
        array[index].color = Colors.redAccent;
        await Future.delayed(
          Duration(milliseconds: speed),
          () {
            setState(() {
        ...
        array[index].color = Colors.yellowAccent;
        index++;
        count[i]--;
      }
    }
  ...
  void saveInputValuesToArray() {
    array = controller.text
        ...
        .split(',')
        .map(
          (e) => ArrayItem(value: int.parse(e), color: Colors.black12),
        )
        .toList();
    setState(() {});
  }
